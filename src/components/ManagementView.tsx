...
export const ManagementView: React.FC<ManagementViewProps> = ({ activeSubView = 'admin' }) => {
    ...
    // Helper: total links in section
    const getTotalLinks = (section: ManagementSection) =>
        section.subGroups.reduce((sum, g) => sum + g.links.length, 0);

    // Flatten sub-group toggle key
    const subKey = (sectionId: string, label: string) => `${sectionId}__${label}`;

    return (
        <div className="management-container">
            <div className="management-sections">
                {sectionsToShow.map((section) => {
                    const hasMultipleGroups = section.subGroups.length > 1;

                    return (
                        <div key={section.id} className={`management-group ${collapsedSections[section.id] ? 'collapsed' : ''}`}>
                            {/* Top-level section header (only in "all sections" view) */}
                            {!activeSubView && (
                                <button
                                ...
                                >
                                    <span className="management-group-title">
                                        ...
                                        {section.title}
                                        <span className="management-group-count">{getTotalLinks(section)}</span>
                                    </span>
                            ...
                            {!collapsedSections[section.id] && (
                                <div className="management-subgroups">
                                    {section.subGroups.map((group) => {
                                        const key = subKey(section.id, group.label);
                                        const isCollapsed = collapsedSections[key];

                                        return (
                                            <div key={group.label} className="management-subgroup">
                                                {/* Sub-group header — only show toggle if section has multiple groups */}
                                                {hasMultipleGroups && (
                                                    <button
                                                        className="management-subgroup-header"
                                                        onClick={() => toggleSection(key)}
                                                    >
                                                        <span className="management-subgroup-title">
                                                            {group.label}
                                                            <span className="management-group-count">{group.links.length}</span>
                                                        </span>
                                                        {isCollapsed
